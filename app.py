...
with st.sidebar:
    ...
    # ── Grouped Navigation (single radio, section headers via markdown) ──────
    all_pages = [
        "— 📊 OVERVIEW —",
        "Welcome / How to Use",
        "Executive Dashboard",
    ...
    # Default to Welcome on first load
    if "_active_page" not in st.session_state:
        st.session_state["_active_page"] = "Welcome / How to Use"

    # Section headers are not selectable — skip them in the radio
    selectable_pages = [p for p in all_pages if not p.startswith("—")]

    page = st.radio(
        "Navigate",
        selectable_pages,
        index=selectable_pages.index(st.session_state["_active_page"])
              if st.session_state["_active_page"] in selectable_pages else 0,
        key="main_nav",
...
page = st.session_state.get("_active_page", "Welcome / How to Use")
